...
def nll_pmf(
    ...
    part3 = (
        sum_.sub(cumsum.gather(1, idx_durations).view(-1))
        .relu()
        .add(epsilon)
        .log()
        .mul(1.0 - events)
...
def _rank_loss_deephit(
    pmf: Tensor, y: Tensor, rank_mat: Tensor, sigma: float, reduction: str = "mean"
) -> Tensor:
...
def nll_pmf_cr(
    ...
    part2 = (
        (1 - sm.cumsum(2)[index, :, idx_durations].sum(1))
        .relu()
        .add(epsilon)
        .log()
        .mul(1 - event_01)
...
def rank_loss_deephit_cr(
    ...
    for i in range(n_risks):
        rank_loss_i = _rank_loss_deephit(
            pmf[:, i, :], y[:, i, :], rank_mat, sigma, "none"
        )
...
def bce_surv_loss(
    ...
    if events.dtype is torch.bool:
        events = events.float()
    y = torch.arange(
        phi.shape[1], dtype=idx_durations.dtype, device=idx_durations.device
    )
    y = (
        y.view(1, -1) < idx_durations.view(-1, 1)
    ).float()  # mask with ones until idx_duration
    c = y + (torch.ones_like(y) - y) * events.view(
        -1, 1
    )  # mask with ones until censoring.
...
def cox_cc_loss(
    ...
    g_control: Tensor,
    shrink: float = 0.0,
    clamp: Tuple[float, float] = (-3e38, 80.0),
) -> Tensor:
    ...
    for ctr in g_control:
        ...
        ctr = torch.clamp(
            ctr, *clamp
        )  # Kills grads for very bad cases (should instead cap grads!!!).
...
def cox_cc_loss_single_ctrl(
    g_case: Tensor, g_control: Tensor, shrink: float = 0.0
) -> Tensor:
...
def cox_ph_loss(
    log_h: Tensor, durations: Tensor, events: Tensor, eps: float = 1e-7
) -> Tensor:
...
class DeepHitSingleLoss(_DeepHitLoss):
    ...
    def forward(
        self, phi: Tensor, idx_durations: Tensor, events: Tensor, rank_mat: Tensor
    ) -> Tensor:
        nll = nll_pmf(phi, idx_durations, events, self.reduction)
        rank_loss = rank_loss_deephit_single(
            phi, idx_durations, events, rank_mat, self.sigma, self.reduction
        )
...
class DeepHitLoss(_DeepHitLoss):
    ...
    def forward(
        ...
        rank_loss = rank_loss_deephit_cr(
            phi, idx_durations, events, rank_mat, self.sigma, self.reduction
        )
        return self.alpha * nll + (1.0 - self.alpha) * rank_loss
...
class CoxCCLoss(torch.nn.Module):
    ...
    def __init__(
        self, shrink: float = 0.0, clamp: Tuple[float, float] = (-3e38, 80.0)
    ) -> Tensor:
        super().__init__()
        self.shrink = shrink
        self.clamp = clamp
